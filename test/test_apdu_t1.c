...
static void build_i_block_response(uint8_t *block, uint32_t *len,
                                   uint8_t ns, const uint8_t *data, uint8_t data_len)
{
    ...
    block[2] = data_len;
    if (data_len > 0)
        common_memcpy(block + 3, data, data_len);
...
void test_apdu_t1_single_exchange(void)
{
    ...
    sc_Status r = protocol_APDU_T1.Transact(&ctx, apdu, sizeof(apdu), recv, &recv_len);
...
void test_apdu_t1_resync_abort(void)
{
     ...
     * Expected path: 3 R-block retries → resync1 (3 retries) → resync2 (3 retries)
     * → resync3 (3 retries) → resyncs > 2 → abort.
     * With the fixed state machine this must terminate, not loop infinitely. */
    uint8_t apdu[] = { 0x00, 0xB0, 0x00, 0x00, 0x01 };
