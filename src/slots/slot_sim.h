...
typedef struct {
    /* RX side: bytes the slot will return when receive_byte/receive_bytes is called */
 ...
 * @param rx      bytes to return from receive_*  (may be NULL if no rx expected)
 * @param rx_len  length of rx
 * @param tx      buffer to capture sent bytes    (may be NULL if tx not inspected)
