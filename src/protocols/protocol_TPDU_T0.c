                                                        ...
                                                        )
{
    ...
    *receive_length = 0;
    ...
    WT = (uint32_t)((float)(context->params.WI * 960 * context->params.D) * ((float)context->params.Fi/context->params.F));
    if (context->params.N == 0xFF){
        GT = 12;
    }else{
        // presence of T=15 protocol
        if (context->params.supported_prot & (0x0001 << SC_PROTOCOL_T15)){
            GT = 12 + (context->params.Fi * context->params.N * context->params.D)/(context->params.F * context->params.Di);
        }else{
    ...
    while (state != TPDU_T0_exit){
        ...
        case TPDU_T0_start_of_transaction   :
            ...
            if (len_to_receive > 2 ){
                if (len_to_receive - 2 != (send_buffer[P3_IDX] == 0 ? 256 : send_buffer[P3_IDX])){
        ...
        case TPDU_T0_receive_procedure_byte :
            ...
            uint8_t inscmplt = (INS ^ 0xFF); // to prevent a bugged warning in gcc, we put operation in tmp var
            if ( proc_byte == inscmplt ||
                 proc_byte == (INS ^ 0xFE) )  // Deprecated)
            {
                if(len_to_send > 0 || len_to_receive > 2) {
...
protocol_itf_t protocol_TPDU_T0 = {
        protocol_TPDU_T0_transact
};
