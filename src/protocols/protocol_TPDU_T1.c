...
static sc_Status    check_Block                         (   uint8_t NAD,
                                                            uint8_t PCB,
                                                            uint8_t LEN,
                                                            uint8_t IFS
                                                        )
    ...
    *receive_length = 0;
    ...
    if (context->params.N == 0xFF){
        ...
        if (context->params.supported_prot & (0x0001 << SC_PROTOCOL_T15)){
            CGT = 12 + (context->params.Fi * context->params.N * context->params.D)/(context->params.F * context->params.Di);
        }else{
            CGT = 12 + context->params.N;
        }
    }

    BWT = 11 + (((0x01 << context->params.BWI)*960*ATR_DEFAULT_F)/context->params.F)*context->params.D;
    BGT = 22;
    ...
    while (state != TPDU_T1_exit){
        ...
        case TPDU_T1_start_of_transaction   :
            ...
            if(    check_Block(    send_buffer[NAD_IDX],
                                send_buffer[PCB_IDX],
                                send_buffer[LEN_IDX],
                                context->params.IFSC)
                != sc_Status_Success)
            ...
            if(len_to_send != (3U + send_buffer[LEN_IDX] + (context->params.EDC == SC_EDC_LRC ? 1U : 2U)))
            {
        ...
        case TPDU_T1_receive_information :
            ...
            ret = slot->receive_bytes(receive_buffer + *receive_length, len_to_receive);
            if (ret != sc_Status_Success){
                END_TRANSACTION(ret);
            }
        ...
        case TPDU_T1_receive_LRC :
            ...
            if (EDC_LRC(receive_buffer, *receive_length) != receive_buffer[*receive_length]){
        ...
        case TPDU_T1_receive_CRC :
            ...
            CRC = (receive_buffer[*receive_length] << 8) | receive_buffer[*receive_length + 1];
        ...
        case TPDU_T1_end_of_transaction :
            ...
            if (ret == sc_Status_Success)
            {
                ret = check_Block(    receive_buffer[NAD_IDX],
                        receive_buffer[PCB_IDX],
                        receive_buffer[LEN_IDX],
                        context->params.IFSD);
            }
...
protocol_itf_t protocol_TPDU_T1 = {
        protocol_TPDU_T1_transact
};
