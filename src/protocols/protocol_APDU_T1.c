...
static sc_Status build_I_block(iso_params_t  *params,
  ...
  *block_len     = T1_PROLOGUE_SIZE;
  ...
  memcpy(block + T1_PROLOGUE_SIZE, data_buffer, len);
  *block_len += len;
...
static sc_Status protocol_APDU_T1_transact(sc_context_t  *context,
  ...
  uint8_t  PCB,
      next_PCB =
          0; /* PCB of the received block and PCB of the next block to send */
  ...
  PCB         = 0;
  memset(snd_block, 0, T1_MAX_BLOCK_SIZE);
  memset(rcv_block, 0, T1_MAX_BLOCK_SIZE);
  ...
  sc_dbg_apdu("T1 APDU >> ", send_buffer, len_to_send);
  ...
  for (;;) {
    ...
    /**
     *  Start of transaction, checking that ifsd have to be negociated and send
     * first Iblock
     */
    ...
    case APDU_T1_transact:
      ...
      ret = protocol_TPDU_T1.Transact(context, snd_block, snd_len, rcv_block,
                                      &rcv_len);
      ...
      if ((get_PCB_type(snd_block[PCB_IDX]) == PCB_S_BLOCK) &&
          (ISSREQ(snd_block[PCB_IDX])) && (get_PCB_type(PCB) != PCB_S_BLOCK)) {
        if (resyncs == 0) {
          state = APDU_T1_prepare_next_block;
          retries++;
        } else {
    ...
    case APDU_T1_process_I_block:
        ...
        /* Last I block don't specified more block */
          ...
          /*if Ns == Nc */
            ...
            memcpy(receive_buffer + *receive_length,
                   rcv_block + T1_PROLOGUE_SIZE, Last_I.LEN);
      ...
      /* If last block was transmitted by the card */
        ...
        /* Ns == Nc */
          ...
          memcpy(receive_buffer + *receive_length, rcv_block + T1_PROLOGUE_SIZE,
                 Last_I.LEN);
          *receive_length += Last_I.LEN;
    ...
    case APDU_T1_process_R_block:
      ...
      if ((rcv_block[LEN_IDX] != 0x00) || ((PCB & 0x2C) != 0x00)) {
        next_PCB = PCB_R_BLOCK | PCB_R_OTHER_ERROR;
        state    = APDU_T1_prepare_next_block;
        retries++;
        break;
    ...
    case APDU_T1_process_S_block:
      ...
      if ((resyncs != 0) && !check_resync_pcb(PCB)) {
        state = APDU_T1_resynch_request;
        break;
      }
      ...
      if (ISSRESP(rcv_block[PCB_IDX])) {
        ...
        if (STYPE(rcv_block[PCB_IDX]) == PCB_S_IFS) {
          ...
          if ((rcv_block[LEN_IDX] == 1) && (rcv_block[3] == snd_block[3])) {
    ...
    case APDU_T1_prepare_next_block:
      ...
      case PCB_I_BLOCK: {
        ret =
            build_I_block(&(context->params), snd_block, &snd_len,
                          send_buffer + send_length, len_to_send - send_length);
      ...
      case PCB_S_BLOCK: {
        ret = build_S_block(&(context->params), next_PCB, snd_block, &snd_len,
                            S_data);
        break;
      }
      }
    ...
    case APDU_T1_end_of_transaction:
      if (ret == sc_Status_Success) {
        sc_dbg_apdu("T1 APDU << ", receive_buffer, *receive_length);
      }
...
protocol_itf_t protocol_APDU_T1 = {protocol_APDU_T1_transact};
