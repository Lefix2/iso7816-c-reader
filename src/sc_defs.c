...
{
    SC_fmaxd, 5000000, 6000000, 8000000, 12000000, 16000000, 20000000, 0, 0, 5000000, 7500000, 10000000, 15000000, 20000000, 0, 0
};

/* Fi table, indicated F for given TA1[8-5] */
static const uint16_t f_table[16] =
{
    SC_Fd, 372, 558, 744, 1116, 1488, 1860, 0, 0, 512, 768, 1024, 1536, 2048, 0, 0
};

/* Di table, indicated D for given TA1[4-1] */
static const uint16_t d_table[16] =
{
    0, SC_Dd, 2, 4, 8, 16, 32, 64, 12, 20, 0, 0, 0, 0, 0, 0
};

static const uint16_t i_table[4] =
{
    25, 50, 100, 0
};
...
uint32_t            get_min_etu_ns                      (   uint8_t iFi,
                                                            uint8_t iDi
                                                        )
{
    uint32_t Fi, Di, fmax;

    if (get_Fi(iFi, &Fi) != sc_Status_Success) return 0;
    if (get_Di(iDi, &Di) != sc_Status_Success) return 0;
    if (get_fmax(iFi, &fmax) != sc_Status_Success) return 0;
