                                                        ...
                                                        )
{
    ...
    /* Init EDC type */
    if((ret=atr_T1_specific_get_EDC(&atr, &(params->EDC))) != sc_Status_Success){
        return ret;
    }

    /* Init IFSD/IFSC */
    if((ret=atr_T1_specific_get_IFS(&atr, &(params->IFSC))) != sc_Status_Success){
        return ret;
    }
    params->IFSD = params->IFSC;

    /* Init CWI, BWI */
    if((ret=atr_T1_specific_get_CBWI(&atr, &(params->CWI), &(params->BWI))) != sc_Status_Success){
        return ret;
    }
                                                        ...
                                                        )
{
    ...
    /* Init fmax */
    if((ret=atr_get_fmax(&(params->ATR), &(params->fmax))) != sc_Status_Success){
            return ret;
    }
...
static sc_Status    prepare_pps                         (   sc_context_t * context,
                                                            uint8_t* pps,
                                                            uint32_t* pps_len
                                                        )
{
    ...
    *pps_len = 2;
    ...
    if (atr->T[0][ATR_INTERFACE_A].present)
    {
        ...
        if( params->Fi/params->Di < ATR_DEFAULT_F/ATR_DEFAULT_D){
            ...
            else if (st != sc_Status_Unsuported_feature) return st;
                                                        ...
                                                        )
{
    ...
    if (pps2_resp_pres)
    {
        ...
        if (pps_resp[PPS0_IDX + pps1_resp_pres + pps2_resp_pres] != pps[PPS0_IDX + pps1_pres + pps2_pres])
            return sc_Status_PPS_Unsuccessfull;
    }
    else
    {
    ...
    if (pps3_resp_pres)
    {
        ...
        if (pps_resp[PPS0_IDX + pps1_resp_pres + pps2_resp_pres + pps3_resp_pres] != pps[PPS0_IDX + pps1_pres + pps2_pres + pps3_pres])
            return sc_Status_PPS_Unsuccessfull;
    }
    else
    {
                                                        ...
                                                        )
{
    ...
    if (    (slot >= SC_MAX_SLOTS) ||
            (!reg_p[slot].registered))
    {
                                                        ...
                                                        )
{
    ...
    do {
        if ((ret = slotContext->slot->activate(current_class)) != sc_Status_Success){
    ...
    if(TA2.present){
        ...
        /* if b5 is present F and D are implicit, this code don't support this feature */
        if (TA2.value & 0x10){
        ...
        ret = protocol_pps.Transact(slotContext, pps, ppslen,pps_resp, &ppsresplen);
        if (ret != sc_Status_Success){
            return ret;
        }
    ...
    ret = slotContext->slot->set_F_D(slotContext->params.F, slotContext->params.D);
    if (ret != sc_Status_Success){
        return ret;
    }
    ...
    dbg_info("  Frequency : %d/%d Hz", slotContext->params.frequency, slotContext->params.fmax);
                                                        ...
                                                        )
{
    ...
    case sc_state_active_on_t0:
        ret = protocol_APDU_T0.Transact(slotContext, send_buffer, send_length, receive_buffer, receive_length);
        break;

    case sc_state_active_on_t1:
        ret = protocol_APDU_T1.Transact(slotContext, send_buffer, send_length, receive_buffer, receive_length);
        break;
